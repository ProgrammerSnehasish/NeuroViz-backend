  ...
  try {
    ...
    if (!userId) {
      await prisma.activityLog.create({
        data: {
          userId: "UNKNOWN",
          action: "UNAUTHORIZED_EMOTION_LOG_ATTEMPT",
          details: `Unauthorized attempt to log emotion.`,
        },
      });
...
export const fetchEmotions = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if(userId !== (req.user as any)?.userId && userId !== (req.user as any)?.id && userId !== (req.user as any)?.sub){
      await prisma.activityLog.create({
        data: {
          userId: (req.user as any)?.userId || (req.user as any)?.id || (req.user as any)?.sub || "UNKNOWN",
          action: "UNAUTHORIZED_FETCH_EMOTION_LOGS_ATTEMPT",
          details: `Unauthorized attempt to fetch emotion logs for user ${userId}.`,
        },
      });

...
export const addFeedback = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!feedback) {
      ...
      throw createHttpError(500, "Could not submit feedback");
    }
    res.status(201).json({
      success: true,
      data: feedback,
    ...
    if (userId !== (req.user as any)?.userId && userId !== (req.user as any)?.id && userId !== (req.user as any)?.sub){
      await prisma.activityLog.create({
        data: {
          ...
          action: "UNAUTHORIZED_FETCH_FEEDBACK_ATTEMPT",
          details: `Unauthorized attempt to fetch feedback for user ${userId}.`,
        },
      });
